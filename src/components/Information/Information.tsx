...
import { storePath } from "../../lib/storeRoute"
...
// 店舗の共有ボタン。Web Share API 対応環境（主にスマホ）では共有シートを開き、
// 非対応環境ではディープリンク(/store/:id)をクリップボードにコピーする。
const ShareButton = ({ cafe }: { cafe: Cafe }) => {
    const [copied, setCopied] = useState(false)

    const handleShare = async () => {
        const url = `${window.location.origin}${storePath(cafe.id)}`

        const zaraz = (window as unknown as { zaraz?: { track: (e: string, p: object) => void } }).zaraz
        zaraz?.track('store_share_clicked', {
            cafe_id: cafe.id,
            cafe_name: cafe.store_name ?? '',
        })

        if (navigator.share) {
            try {
                await navigator.share({
                    title: `${cafe.store_name ?? 'グルメインフォ'} | グルメインフォ`,
                    url,
                })
            } catch {
                // 共有シートのキャンセルは何もしない
